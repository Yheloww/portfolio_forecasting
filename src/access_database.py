...
def df_per_tickers(ticker, con):
    """
    """
    df = pd.read_sql(f'SELECT  Date,"Adj Close**" fROM {ticker}',
                       con,
    ...
    df2 = df.loc[mask]
    df2 = df2.rename(columns={"Adj Close**": f"{ticker}"})
...
df2 = complete_df(Portfolio1,con)
plot(df2)
